...
function Body() {
    ...
    return (
        <div>{loading ? <Loading /> : <div className=' grid grid-cols-1 sm:grid-cols-4 ml-10 mt-10 gap-y-5  place-items-center '>
            {isLogin ? perfumes?.map((perf) => <Image name={perf.name} url={perf.image_url} price={perf.price} category={'category'} perfume_id={perf._id} />) : perfumes?.slice(0, 6).map((perf) => <Image name={perf.name} url={perf.image_url} price={perf.price} category={'category'} perfume_id={'##'} />)}
